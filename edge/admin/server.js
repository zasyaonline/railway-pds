...
const { getAnnounceRuntime, startAnnounceService } = require('../announce/service');
...
function createAdminApp() {
  ...
  app.get('/api/admin/status', (req, res) => {
    ...
    res.json({
      ...health,
      announce: getAnnounceRuntime().status(),
      actor: 'local-admin',
  ...
  app.post('/api/admin/announcements/auto', (req, res) => {
    ...
    recordAudit({ action: 'announce_auto', details: { enabled } });
    res.json(getAnnounceRuntime().setAuto(enabled));
  });

  app.post('/api/admin/announcements/pause', (req, res) => {
    if (!requireAdmin(req, res, log)) return;
  ...
  app.post('/api/admin/announcements/volume', (req, res) => {
    if (!requireAdmin(req, res, log)) return;
    res.json(getAnnounceRuntime().setVolume(req.body?.volume));
  });

  app.post('/api/admin/announcements/manual', (req, res) => {
    if (!requireAdmin(req, res, log)) return;
    const job = getAnnounceRuntime().enqueueManual(req.body || {});
    recordAudit({
      action: 'announce_manual',
      details: { type: job.type, trainNo: job.trainNo }
    });
    res.json({ ok: true, job, ...getAnnounceRuntime().status() });
  });

  app.post('/api/admin/announcements/cancel', (req, res) => {
    if (!requireAdmin(req, res, log)) return;
    const result = getAnnounceRuntime().cancel(req.body?.id);
    res.json({ ...result, ...getAnnounceRuntime().status() });
  });

  app.post('/api/admin/announcements/replay', async (req, res) => {
    if (!requireAdmin(req, res, log)) return;
    try {
      const result = await getAnnounceRuntime().replay(req.body?.id);
      if (result?.error === 'history item not found') {
        return res.status(404).json(result);
      }
      res.json({ ok: true, result, ...getAnnounceRuntime().status() });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });
...
function startAdminServer() {
  ...
  if (isAppliance() || process.env.RAILWAY_ANNOUNCE === '1') {

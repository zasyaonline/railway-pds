...
function copyDirIfExists(src, dest) {
  ...
  for (const name of fs.readdirSync(src)) {
    ...
    if (fs.statSync(from).isDirectory()) copyDirIfExists(from, to);
...
function exportPackage() {
  ...
  copied.overlays = copyDirIfExists(overlayDir(), path.join(dest, 'overlays'));
  const manifest = {
...
function restorePackage(srcDir) {
  ...
  copyDirIfExists(path.join(srcDir, 'overlays'), overlayDir());

...
function overlayDir() {
  return path.join(etcDir(), 'overlays');
}

function overlayStationDir(code) {
  const c = String(code || '').trim().toUpperCase();
  return path.join(overlayDir(), c || '_');
}

function ttsDataDir() {
  return path.join(dataRoot(), 'tts');
}

function ttsCacheDir() {
  return path.join(ttsDataDir(), 'cache');
}

function announceDataDir() {
  return path.join(dataRoot(), 'announce');
}

function announceHistoryPath() {
  return path.join(announceDataDir(), 'history.json');
}

function announceRuntimePath() {
  return path.join(runtimeDir(), 'announce_state.json');
}

function announceMemoryPath() {
  return path.join(announceDataDir(), 'memory.json');
}

function coachSourceDataDir() {
  return process.env.COACH_DATA_DIR || path.join(__dirname, '..', 'coach-position', 'data');
}
